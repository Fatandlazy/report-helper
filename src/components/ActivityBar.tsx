...
export function ActivityBar({ active, onChange }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full flex justify-center mt-auto mb-2">
        {active === "settings" && (
          <span
            className="absolute left-0 top-2 bottom-2 rounded-r"
            style={{ width: 2, background: "#007acc" }}
        ...
        <button
          ...
          style={{
            ...
            alignItems: "center",
            justifyContent: "center",
            color: active === "settings" ? "#cccccc" : hovered === "settings" ? "#cccccc" : "#858585",
          }}
        >
          <span className="codicon codicon-settings" style={{ fontSize: 22 }} />
        </button>
      </div>
    </div>
  );
